...
async def handle_login(
	...
	user = crud.get_user_by_email(db, email=username)  # 주석이 유저 정보를 이메일 주소로 받아온다.
	if not user or not security.verify_password(password, user.hashed_password):  # 이메일 주소로 조회한 정보가 있다면 암호를 검증한다.
		return templates.TemplateResponse(
			"login.html",
